...
async def repeat(ctx, *, message):
    if ctx.channel.id in spam_tasks:
        await ctx.send("Already repeating in this channel.")
        return
    ...
    repeater.start()
    spam_tasks[ctx.channel.id] = repeater

    await ctx.send("Started repeating.")

@bot.command()
async def stop(ctx):
    task = spam_tasks.pop(ctx.channel.id, None)
    if task:
        task.cancel()
        await ctx.send("Stopped.")
    else:
        await ctx.send("Nothing is repeating.")
